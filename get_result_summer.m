function [ result ] = get_result_summer( Xn,summer_typical_day )
...
for index = 1:var_number:1+(day_period-1)*var_number
    ...
    Constraints = [Constraints, temp(1) == pv_capactiy*summer_typical_day(ceil(index/var_number),3)];
    Constraints = [Constraints, temp(1) + temp(2) + temp(6) + temp(11) ...
        == summer_typical_day(ceil(index/var_number),1) + temp(3)/cop_c_hp + temp(5)];
...
%         + 0.8365*(temp(2)/cchp_capacity) + 0.0926;
    ...
    Constraints = [Constraints, temp(3) + cchp_cold + temp(8) ...
        == summer_typical_day(ceil(index/var_number),2) + temp(7)];
    % 储能约束
%     Constraints = [Constraints, temp(5)*temp(6) == 0];
%     Constraints = [Constraints, temp(7)*temp(8) == 0];
    Constraints = [Constraints, temp(4) == 0];
    Constraints = [Constraints, [temp(9) temp(10)] == [0 0]];    %夏季没有制热
    % 储能容量约束
...
if(sol.problem == 0)
%     'Optimization is successful : 2'
%     flag=1;
%     value(x);
    result=value(x);
else
%     disp('Optimization is failed!2');
    sol.problem;
    sol.info;
    result=0;
%     flag=0;
