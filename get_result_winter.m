function [result] = get_result_winter( Xn,winter_typical_day )
...
for index = 1:var_number:1+(day_period-1)*var_number
    ...
    Constraints = [Constraints, temp(1) == pv_capactiy*winter_typical_day(ceil(index/var_number),3)];
    Constraints = [Constraints, temp(1) + temp(2) + temp(6) + temp(11) ...
        == winter_typical_day(ceil(index/var_number),1) + temp(3)/cop_h_hp + temp(5)];
...
%         + 0.8365*(temp(2)/cchp_capacity) + 0.0926;
    ...
    Constraints = [Constraints, temp(3) + temp(4) + cchp_heat + temp(10) ...
        == winter_typical_day(ceil(index/var_number),2) + temp(9)];
    % 储能约束
...
if(sol.problem == 0)
%     'Optimization is successful : 1'
    
%     value(x);
    result=value(x);
else
%     disp('Optimization is failed!1');
    sol.problem;
    sol.info;
    result=0;
end
