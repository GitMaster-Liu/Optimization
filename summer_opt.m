...
for index = 1:var_number:1+(day_period-1)*var_number
    ...
    Constraints = [Constraints, temp(1) == pv_capactiy*summer_typical_day(ceil(index/var_number),3)];%光伏约束
    Constraints = [Constraints, temp(1) + temp(2) + temp(6) + temp(11) ...
        == summer_typical_day(ceil(index/var_number),1) + temp(3)/cop_c_hp + temp(5)];%电负荷约束 地源热泵用于制冷
    cchp_cool = 0.85*(temp(2)*(1 - cchp_yita - cchp_yita_dis))*cop_c_cchp/cchp_yita;
    Constraints = [Constraints, cchp_cool + temp(3) + temp(8)  ...%这里的temp(3)是地缘热泵制冷功率
        == summer_typical_day(ceil(index/var_number),2) + temp(7)];%冷负荷约束
...
%         + 0.8365*(temp(2)/cchp_capacity) + 0.0926;%非线性，不能用
    ...
    Constraints = [Constraints, [temp(9) temp(10)] == [0 0]];    %夏季没有制热,热负荷约束
    % 储能约束
