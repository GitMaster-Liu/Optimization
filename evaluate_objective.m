function f = evaluate_objective(N, chromosome, follow,winter_typical_day,summer_typical_day,transition_typical_day)
...
for i=1:24
    ...
    E_c_dem=E_c_dem+122*summer_typical_day(i,2)+123*transition_typical_day(i,2);%冷总负荷
...
for day=1:120
    for i=1:24
        P_je(i+24*day-24)=winter_typical_day(i,1)-chromosome(2+4*i-4);%电净负荷=负荷-放电+充电
        P_jc(i+24*day-24)=0;%冷净负荷
        P_jh(i+24*day-24)=winter_typical_day(i,2)-follow(4+5*i-5);%热净负荷
    end
end
%夏季
for day=121:242
    for i=1:24
        P_je(i+24*day-24)=summer_typical_day(i,1)-chromosome(98+3*i-3);%电净负荷=负荷-放电+充电
        P_jc(i+24*day-24)=summer_typical_day(i,2)-follow(124+5*i-5);%冷净负荷
...
for day=243:365
    for i=1:24
        P_je(i+24*day-24)=transition_typical_day(i,1)-chromosome(170+4*i-4);%电净负荷=负荷-放电+充电
        P_jc(i+24*day-24)=transition_typical_day(i,2)-follow(244+6*i-6);%冷净负荷
        P_jh(i+24*day-24)=transition_typical_day(i,3)-follow(246+6*i-6);%热净负荷
    end
end
sum_je=0;%电净负荷变化平方和
sum_jc=0;%冷净负荷变化平方和
