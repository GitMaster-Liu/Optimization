function [result] = winter_opt( pv_capactiy,cchp_capacity,hp_capacity, gb_capacity,es_capacity,...
...
for index = 1:var_number:1+(day_period-1)*var_number
    ...
    Constraints = [Constraints, temp(1) == pv_capactiy*winter_typical_day(ceil(index/var_number),3)];%光伏约束
    Constraints = [Constraints, temp(1) + temp(2) + temp(6) + temp(11) ...
        == winter_typical_day(ceil(index/var_number),1) + temp(3)/cop_h_hp + temp(5)];%电负荷约束
    Constraints = [Constraints, [temp(7) temp(8)] ...
        == [0 0]]%冷负荷约束
%     cchp_yita = 0.4166*(temp(2)/cchp_capacity)^3 - 1.0135*(temp(2)/cchp_capacity)^2 ...
%         + 0.8365*(temp(2)/cchp_capacity) + 0.0926;%非线性，不能用
    cchp_heat = 0.85*(temp(2)*(1 - cchp_yita - cchp_yita_dis))*cop_h_cchp/cchp_yita;
    Constraints = [Constraints, temp(3) + temp(4) + cchp_heat + temp(10) ...
        == winter_typical_day(ceil(index/var_number),2) + temp(9)];%热负荷约束
    % 储能约束
